...
//    C:\Users\Ja\Downloads\oldsrcfiles\IM_TNA_monthly_042016_www.open-water.org.uk-20160415110019-00000.cdx
//    C:\Users\Ja\Downloads\oldsrcfiles\IM_TNA_monthly_042016_www.open-water.org.uk-20160415110019-00000.arc.gz
...
dnsfiles = "C:\\Users\\Ja\\Google Drive\\oldfiles\\Note\\srcfile\\SearchWhitelist\\dns_*"
   ...
   // System.setProperty("HADOOP_USER_NAME", username)
    ...
    val zipines = rddLines.zipWithIndex

    val cdxItems_offsets = zipines.map(x =>
      {
        offsetdetails(x._2,  x._1.split(" ")(2), x._1.split(" ")(9), "")
      })
    var lstOffsets = new ListBuffer[offsetdetails]
    var lstOffsetsF = new ListBuffer[offsetdetails]

    cdxItems_offsets.collect().foreach(x => {
      lstOffsets += offsetdetails(x.itemno,  x.url, x.offset,x.offsetto)
    })

    lstOffsets = lstOffsets.sortBy(x => x.offset)

    cdxItems_offsets.collect().foreach(x => {
      var offsetto = "0"
      val head = lstOffsets.filter(y => y.offset > x.offset).take(1)
      if(!head.isEmpty)
        offsetto = (head.head.offset.toLong - 1).toString
      lstOffsetsF += offsetdetails(x.itemno,  x.url, x.offset,offsetto)
    })


    val offsetdf = JobSparkConf.sc.parallelize(lstOffsetsF.toList, 1)
    val offdf = offsetdf.toDF()
    offdf.registerTempTable("offsets")



    offdf.show(10, truncate = false)
  ...
  println("s")
    ...
    val filteredCDXitems_1 = cdxItems1.filter(!cdxItems1("massaged_url").equalTo("")).toDF()
    val filteredCDXitems1 = filteredCDXitems_1.filter(cdxItems1("massaged_url").contains("doc")).toDF()
    filteredCDXitems1.show(10)
 ...
 /*   val file = new File(arcfile)
   ...
   // val file = new File(arcfile)                                 //  val file1 = new File(arcfile)   thinking about hadoop files..this will work for local .. not for hadoop
    ...
    val lstFinalDataToSave = joinedDF.collect().map(cdxItem => {
      ...
      //try {
        if(mime.equals("application/msword") || mime.equals("application/vnd.ms-excel") || mime.contains("vnd.ms-powerpoint")   )//if(url.equals("http://www.courtservice.gov.uk/cms/media/form-pra.doc"))
           {
             println("break")
           }
         ...
         if(mime.contains("text/html") || mime.contains("text/xml"))
           {
             bodyBytes = getArcRecordBody(arcrecord, 0)
            // imageData = new String(bodyBytes, Charset.forName("UTF-8"))
             imageData =  jatikaparser.parseHtmlFile(bodyBytes).replaceAll("\\s{2,}", " ").trim
           }
         ...
         else if(mime.contains("pdf"))
           {
             ...
             val matchitemendbyte = offdf.filter(offdf("url").equalTo(url)).collect().head.getAs[String]("offsetto")
             var endbytes : Long = 0
             if(!matchitemendbyte.isEmpty) endbytes = matchitemendbyte.toLong

              println("end bytes in pdf : " + endbytes)

             bodyBytes = getArcRecordBody(arcrecord, endbytes)
             //imageData = new String(bodyBytes, Charset.forName("UTF-8"))


             imageData =  jatikaparser.parsePdfFileWithpbox(bodyBytes)
               //.parsePdfFile(bodyBytes).replaceAll("\\s{2,}", " ").trim
           }
...
//1 sec sir .. complete the final step
  ...
  def getArcRecordBody(arcrecord: ArchiveRecord, endbytes: Long) : Array[Byte] = {
    ...
    if(original.split("\r\n").length > 0)
    {
      if( original.split("\r\n")(0).contains("Content-type")  || original.split("\r\n")(0).contains("Last-modified"))
        delimiter = "\n"
    }


    var lines = original.split(delimiter)
    /*HTTP/1.0 200 OK
      Date: 2005-02-16T12:00:00Z
      Content-type: application/pdf
    Last-modified: 2005-02-16T12:00:00Z*/
    ...
    var fkendbyte = os.toByteArray.length //if(endbytes <= 0) os.toByteArray.length else endbytes.toInt


    val bytesize = fkendbyte - messagelength

    var newBytes = new Array[Byte](bytesize)
  ...
  //  println("Bytes length :" + messageops.getBytes().length + " message length:" + messagelength)
    try {
       newBytes = java.util.Arrays.copyOfRange(bytes, messagelength, fkendbyte)

       org.apache.commons.io.FileUtils.writeByteArrayToFile(new File("C:\\Users\\Ja\\Google Drive\\srcfile\\esoutput_json\\docxtest.docx"), newBytes)
      println("stop")
    }
    catch
      {
